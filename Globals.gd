...
# Gets set by the Credentials_Manager
...
#var cred_manager : Credentials_Manager

var crypto = Crypto.new()

var state = "Temp" #setget , create_state

var cred_manager = Credentials_Manager.new()
...
func save_data():
	...
	var user = cred_manager.user
	
	if credentials[user].has("token"):
		
		result = cred_manager.save_creds(credentials[user]["token"], 
...
#	yield(cred_manager, "creds_saved")
